...
class MapFileExporter:
    ...
    def save_rivers_bmp(self, rivers_bmp: np.ndarray) -> str:
        ...
        # Sea background (grey, index 8) first, so the river colours below
        # correctly paint over it wherever a river crosses a coastal pixel.
        sea_bg = np.all(rivers_bmp == np.array([122, 122, 122], dtype=np.uint8), axis=-1)
        idx_img[sea_bg] = 8
        ...
        for rgb in RIVER_PALETTE:
            palette.extend(rgb)
        palette += [122, 122, 122]  # index 8: sea background
        palette += [0, 0, 0] * (256 - len(RIVER_PALETTE) - 1)
        pal_img.putpalette(palette)
    ...
    def save_trees_bmp(self, terrain_bmp: np.ndarray) -> str:
        """Paints trees.bmp using a palette and density calibrated against a
        real EU4 trees.bmp: sampling one showed ~15% land coverage across 4
        distinct non-black tree-type colors (not a solid fill of every
        wooded pixel), roughly:
          - a dark teal, by far the most common, tracing temperate forest
            coastlines (Canada, Russia, Northern Europe)
          - a bright green in solid tropical blotches (Central America)
          - a purple, concentrated in one region (South Asia) -- a distinct
            monsoon/jungle tree type
          - a rare orange accent, only a handful of pixels
        EU4 also refuses to load a trees.bmp with zero tree pixels at all.
        """
        from scipy.ndimage import gaussian_filter
        from engine.map_generation import TerrainClassifier
        colors = TerrainClassifier.TERRAIN_COLORS

        def terrain_mask(name):
            rgb = np.array(colors[name], dtype=np.uint8)
            return np.all(terrain_bmp == rgb, axis=-1)

        forest_mask = terrain_mask("forest")
        jungle_mask = terrain_mask("jungle")
        h, w = terrain_bmp.shape[:2]
        rng = np.random.RandomState(20260826)

        def organic_field(scale):
            """Smoothed noise upsampled to full size -- gives soft, blobby
            region edges instead of a visibly geometric block grid."""
            small = rng.random_sample((max(2, h // scale), max(2, w // scale)))
            smooth = gaussian_filter(small, sigma=1.4)
            smooth = (smooth - smooth.min()) / (smooth.max() - smooth.min() + 1e-9)
            return np.array(Image.fromarray((smooth * 255).astype(np.uint8))
                             .resize((w, h), Image.BILINEAR)) / 255.0

        include_field = organic_field(48)
        color_field = organic_field(64)
        accent_field = organic_field(20)
        pixel_density = rng.random_sample((h, w))

        # Forest sits in a much narrower latitude band than jungle in this
        # generator, so its inclusion threshold is relaxed to keep the two
        # roughly proportionate to the reference (teal as the most common).
        include_forest = include_field < 0.55
        include_jungle = include_field < 0.35
        accent_zone = accent_field > 0.985
        dense = pixel_density < 0.8

        idx_img = np.zeros((h, w), dtype=np.uint8)
        idx_img[forest_mask & include_forest & dense & ~accent_zone] = 1
        idx_img[jungle_mask & include_jungle & dense & (color_field < 0.55) & ~accent_zone] = 2
        idx_img[jungle_mask & include_jungle & dense & (color_field >= 0.55) & ~accent_zone] = 3
        idx_img[(forest_mask | jungle_mask) & include_forest & dense & accent_zone] = 4
        ...
        pal_img = Image.fromarray(idx_img, mode="P")
        palette = [
            0, 0, 0,        # 0 no tree -- black, matches reference background
            10, 65, 45,      # 1 temperate forest -- dark teal
            10, 130, 10,      # 2 tropical forest -- bright green
            140, 60, 195,      # 3 monsoon/jungle forest -- purple
            180, 160, 50,       # 4 rare accent forest -- orange
        ] + [0, 0, 0] * 251
        pal_img.putpalette(palette)
    ...
    def write_default_map(self, width: int, height: int, province_infos: List,
                           tree_indices=(1, 2, 3, 4), lake_ids=()) -> str:
        ...
        lines = [
            ...
            "lakes = { " + " ".join(str(i) for i in sorted(lake_ids)) + " }",
            "force_coastal = { }",
            "",
            ...
            "",
            f"tree = {{ {' '.join(str(i) for i in tree_indices)} }}",
        ]
    ...
    def write_terrain_txt(self) -> str:
        ...
        lines.append("")
        lines.append("tree = {")
        lines.append("    temperate_forest_tree = {")
        lines.append("        terrain = forest")
        lines.append("        color = { 1 }")
        lines.append("    }")
        lines.append("    tropical_forest_tree = {")
        lines.append("        terrain = jungle")
        lines.append("        color = { 2 }")
        lines.append("    }")
        lines.append("    monsoon_forest_tree = {")
        lines.append("        terrain = jungle")
        lines.append("        color = { 3 }")
        lines.append("    }")
        lines.append("    accent_forest_tree = {")
        lines.append("        terrain = forest")
        lines.append("        color = { 4 }")
        lines.append("    }")
        lines.append("}")
    ...
    def append_canal_adjacencies(self, rows: List[str]) -> None:
        """Appends extra rows (e.g. canal-type entries) to the already
        written adjacencies.csv."""
        if not rows:
            return
        path = f"{self.map_dir}/adjacencies.csv"
    ...
    def append_canal_definitions(self, blocks: List[str]) -> None:
        """Appends canal_definition = {...} blocks to the already written
        default.map, as required for each canal's river tile to show up."""
        if not blocks:
            return
        path = f"{self.map_dir}/default.map"
        with open(path, "a", encoding="utf-8") as f:
    ...
    def append_ambient_objects(self, blocks: List[str]) -> None:
        """Appends dummy ambient_object entries (one per canal, keyed by
        the canal's name) to the already written ambient_object.txt."""
        if not blocks:
            return
        path = f"{self.map_dir}/ambient_object.txt"
        with open(path, "a", encoding="utf-8") as f:
            f.write("\n")
            for block in blocks:
                f.write(block + "\n")
...
class MasterExportOrchestrator:
    ...
    def export_complete_mod(self, mod_name: str, heightmap: np.ndarray, land_mask: np.ndarray,
        ...
        lake_ids = [p.id for p in province_infos if p.is_lake]
        results["default_map"] = map_exp.write_default_map(width, height, province_infos, lake_ids=lake_ids)
        ...
        # ---- canals (land-bridge/isthmus/peninsula/lake-connector analysis) ----
        from engine.canal_detection import CanalDetector
        canal_candidates = CanalDetector.detect(land_mask, province_infos, max_canals=8)
        if canal_candidates:
            h_map = land_mask.shape[0]
            canal_rows = []
            for c in canal_candidates:
                gy = h_map - c.y  # EU4's y axis is bottom-up, like positions.txt
                canal_rows.append(
                    f"{c.province_a};{c.province_b};canal;-1;{c.x};{gy};{c.x};{gy};{c.name} ({c.note})"
                )
            map_exp.append_canal_adjacencies(canal_rows)
        results["canal_candidates"] = canal_candidates
        ...
        # ---- decisions / events / missions / common / localisation / history templates ----
        # (Form-nation decisions, Celestial Directorate + Hindu-dominance +
        # tech-inversion events, Celestial Directorate/Hindu/African mission
        # trees, national ideas boilerplate, bookmark, advisor pools, HRE
        # diplomacy template. Deliberately excludes vanilla nation-specific
        # content -- e.g. "Form Great Britain", "Restore Byzantine Empire",
        # the Polish/Russian/Turkish/etc mission trees -- since those tags
        # don't exist in a procedurally generated world.)
        from export.template_exporter import TemplateExporter, TemplateExportConfig
        template_exp = TemplateExporter(
            config=TemplateExportConfig(mod_name=mod_name, mod_path=tech_name,
                                         starting_date="1444.11.11"),
            templates_dir=str(eu4_wgs_v8.TEMPLATES_DIR),
        )
        results["templates"] = template_exp.export_all(mod_root)
