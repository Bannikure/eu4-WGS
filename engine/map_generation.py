...
class TerrainClassifier:
    ...
    def __init__(self, width_or_heightmap=None, height: int = None):
        # Handle case where studio.py passes the heightmap array as the first argument
        if isinstance(width_or_heightmap, np.ndarray):
            self.heightmap = width_or_heightmap
            h, w = width_or_heightmap.shape
            self.width = w
            self.height = h
        else:
            self.heightmap = None
            self.width = width_or_heightmap if width_or_heightmap is not None else 2048
            self.height = height if height is not None else 2048

    def generate_terrain_bmp(self, heightmap: np.ndarray = None, land_mask: np.ndarray = None) -> np.ndarray:
        """Generates terrain.bmp mapping climate zones to indexed colors."""
        # Fallback to stored heightmap if none is explicitly passed
        if heightmap is None:
            heightmap = self.heightmap
            
        if heightmap is None:
            raise ValueError("No heightmap provided to TerrainClassifier!")

        # Fallback to our RGB 87 land threshold if land_mask is missing
        if land_mask is None:
            land_mask = heightmap > 87
