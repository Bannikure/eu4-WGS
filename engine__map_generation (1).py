...
def parse_args():
    ...
    parser.add_argument("--map-style", type=str, default="random",
                        choices=["random", "pangea", "continents", "archipelago",
                                 "continents_islands", "earth_like", "island_grid",
                                 "fantasy_ring", "fantasy_spine", "shattered"],
                        help="Map style (default: random -- picks a different style "
                             "per seed instead of always the same one)")
...
def run_headless_pipeline(args):
    ...
    export_result = orchestrator.export_complete_mod(
        ...
        normal_map=result.get("normal_map"),
    )
